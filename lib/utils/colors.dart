import 'dart:ui';

class ThemeColors {
  ThemeColors._internal();
  static const Color mainTitle = Color(0xFF000000);
  static const Color subTitle = Color(0xFF7B7B7B);
  static const Color txtFieldBorderColor = Color(0xFFE1E1E1);
  static const Color btnSignInSignUp = Color(0xFFBcB4FF);
  static const Color dividerOrSIgnInWith = Color(0xFFC8C8C8);
  static const Color socialSignInButton = Color(0xFFE9EEED);
  static const Color toggleSwitchBgColor = Color(0xFFBCB4FF);
  static const Color toggleSwitchButtonColor = Color(0xFF433C90);
  static const Color toggleSwitchBorderColor = Color(0xFF433C90);
  static const Color toggleSwitchActiveButtonColor = Color(0xFF433C90);
  static const Color toggleSwitchInActiveButtonColor = Color(0xFF474747);
  static const Color errorFontColor = Color(0xFFEE1111);
  static const Color backButtonBackgroung = Color(0xFFFFFFFF);
  static const Color backButtonBorderColor = Color(0xFFE1E1E1);
  static Color wishCardButtonColor = const Color(0xFFFFB985);

  static const Color otpTextColor = Color(0xFFA2A2A2);
  static const Color resendTextColor = Color(0xFF333333);
  static const Color otpCountdownTmerColor = Color(0xFF868686);
  static const Color resendUnderLineColor = Color(0xFFD9D9D9);
  static const Color chatRoundDotcolor = Color(0xFF696969);
  static const Color loginNowTextColor = Color(0xFF2458D5);
  static Color wishCardBgColor = const Color(0xFFF37253).withOpacity(0.1);
  static Color tripCardBgColor = const Color(0xFFBCB4FF).withOpacity(0.2);
  static Color checkDeliveryUnderlineColor = Color(0xFFE0E0E0);
  static const Color cardMainTitleColor = Color(0xFF414141);
  static Color toggleSwitchinActiveBorderColor =
      Color(0xFFF1C1C1C).withOpacity(0.15);

  static Color bottomNavSelectedBgColor = const Color(0xFFBCB4FF);
  static Color bottomNavUnSelectedLabelColor = const Color(0xFFABABAB);

  static Color bottomModelSheetBgtranspearentColor =
      Color(0xFFCDCDCD).withOpacity(0.85);
}
